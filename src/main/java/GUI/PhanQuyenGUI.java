...
public class PhanQuyenGUI {
    ...
    panel_buttons.add(btn_add);panel_buttons.add(btn_edit);panel_buttons.add(btn_delete);

    // Tạo bảng
    String[] columnNames = { "MÃ QUYỀN", "TÊN QUYỀN" };model=new DefaultTableModel(columnNames,0);

    refreshTable(); // Load initial data from database
    table=new JTable(model);table.setRowHeight(25);table.getTableHeader().setResizingAllowed(false);table.getTableHeader().setReorderingAllowed(false);table.setDefaultEditor(Object.class,null);table.setShowGrid(true);table.setRowSelectionAllowed(true);table.setSelectionBackground(new Color(240,240,240));table.setSelectionForeground(Color.BLACK);table.setFont(new Font("Arial",Font.PLAIN,14));table.setBackground(Color.white);table.getTableHeader().setFont(new Font("Arial",Font.BOLD,18));table.getTableHeader().setBackground(MENU_BACKGROUND);table.getTableHeader().setForeground(Color.WHITE);table.setCursor(new Cursor(Cursor.HAND_CURSOR));table.setDragEnabled(false);table.setFocusable(false);table.setBackground(Color.WHITE); // Changed
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          // from
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          // MENU_BACKGROUND
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          // for
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          // better
    ...
    DefaultTableCellRenderer centerRenderer = new DefaultTableCellRenderer();centerRenderer.setHorizontalAlignment(JLabel.CENTER);table.getColumnModel().getColumn(0).setCellRenderer(centerRenderer);table.getColumnModel().getColumn(1).setCellRenderer(centerRenderer);if(table.getRowCount()>0)
    {
        table.setRowSelectionInterval(0, 0);
    }
    JScrollPane scrollPane = new JScrollPane(table);panel_table.add(scrollPane,BorderLayout.CENTER);

    panel.add(panel_table,BorderLayout.CENTER);panel.add(panel_buttons,BorderLayout.NORTH);
    ...
    private void deleteQuyen() {
        ...
        int confirm = JOptionPane.showConfirmDialog(panel, "Bạn có chắc chắn muốn xóa quyền này?", "Xác nhận xóa",
    ...
    private void updateQuyen() {
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(panel, "Vui lòng chọn quyền để sửa!", "Thông báo",
    ...
    private void deleteQuyen() {
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(panel, "Vui lòng chọn quyền để xóa!", "Thông báo",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
        int maQuyen = Integer.parseInt(table.getValueAt(selectedRow, 0).toString());
        ...
        if (result == JOptionPane.YES_OPTION) {
            if (quyenBUS.deleteQuyen(maQuyen)) {
                JOptionPane.showMessageDialog(panel, "Xóa quyền thành công!", "Thông báo",
